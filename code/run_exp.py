# -*- coding: utf-8 -*-
"""
Run at-home category learning experiment using PsychoPy.
"""
...
PULSE_MS_SHORT = 20  # A stimuli/responses and correct feedback
PULSE_MS_LONG = 40   # B stimuli/responses and incorrect feedback
...
if __name__ == "__main__":
    ...
    while True:
        lab_day_prompt.text = (
            "Did you complete a lab day today?\n\n"
            f"Answer: {lab_day_answer or '___'}\n\n"
            "Type yes or no, then press ENTER.\n"
            "Press BACKSPACE to edit or ESC to quit."
            f"{lab_day_error}"
        )
        lab_day_prompt.draw()
        win.flip()
        ...
        for key in keys:
            if key == "escape":
                ...
                sys.exit()
            if key == "backspace":
                lab_day_answer = lab_day_answer[:-1]
                lab_day_error = ""
    ...
    session_seed = stable_int_seed(
        f"{subject}_{session_num:03d}_exp_{session_tag}")
    ...
    while running:
        ...
        # --------------------- STATE: STIM ---------------------
            ...
            if state_entry:
                if phase == 'train':
                    if cat == "A":
                        width_ms = PULSE_MS_SHORT
                        trig = TRIG["STIM_ONSET_A"]
                    else:
                        width_ms = PULSE_MS_LONG
                ...
                if not np.isnan(trig):
                    trigger_port.flip_pulse(
                        trig, width_ms=width_ms, global_clock=global_clock)
                    trig_stim = int(trig)
        ...
        # --------------------- STATE: FEEDBACK ---------------------
            ...
            if time_state > 1000:
                ...
                trial_data["session_tag"].append(session_tag)
                trial_data["lab_day_completed_today"].append(lab_day_answer)
                trial_data["planned_trials"].append(n_total)
