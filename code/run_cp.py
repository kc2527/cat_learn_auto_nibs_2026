...
TMS_TRIGGER_PORT_ADDRESS = "0x3FB8"
TMS_TRIGGER_DEFAULT_PULSE_MS = 50
...
if __name__ == "__main__":
    ...
    while running:
        if default_kb.getKeys(keyList=["escape"], waitRelease=False):
            trigger_port.pulse_now(TRIG["EXP_END"], global_clock=global_clock)
            break
        ...
        # --------------------- STATE: PRACTICE INTRO ---------------------
            ...
            keys = kb.getKeys(keyList=["space"], waitRelease=False, clear=True)
            if keys:
                trigger_port.flip_pulse(TRIG["CP_PRACTICE_START"],
        ...
        # --------------------- STATE: MAIN INTRO ---------------------
            ...
            if state_entry:
                if practice_started and not practice_finished:
                    trigger_port.pulse_now(TRIG["CP_PRACTICE_END"],
                                  global_clock=global_clock)
            ...
            keys = kb.getKeys(keyList=["space"], waitRelease=False, clear=True)
            if keys:
                trigger_port.flip_pulse(TRIG["CP_MAIN_BLOCK_START"],
                               global_clock=global_clock)
                main_started = True
                state_current = "state_iti"
                state_entry = True
        ...
        # --------------------- STATE: ITI ---------------------
            ...
            if state_entry:
                state_clock.reset()
                trigger_port.flip_pulse(TRIG["CP_ITI_ONSET"], global_clock=global_clock)
                next_trial = trial + 1
                ...
                if trial >= len(trials):
                    if not practice_finished and practice_started:
                        trigger_port.pulse_now(TRIG["CP_PRACTICE_END"],
                                      global_clock=global_clock)
                    if main_started:
                        trigger_port.pulse_now(TRIG["CP_MAIN_BLOCK_END"],
                                      global_clock=global_clock)
                    state_current = "state_finished"
                    state_entry = True
                else:
        ...
        # --------------------- STATE: INTERVAL 1A ---------------------
            ...
            if state_entry:
                ...
                if not np.isnan(trig_i1):
                    trigger_port.flip_pulse(int(trig_i1), global_clock=global_clock)
        ...
        # --------------------- STATE: INTERVAL 2A ---------------------
            ...
            if state_entry:
                state_clock.reset()
                trigger_port.flip_pulse(TRIG["CP_INTERVAL2_ONSET"],
                               global_clock=global_clock)
        ...
        # --------------------- STATE: RESPONSE ---------------------
            ...
            if state_entry:
                ...
                trigger_port.flip_pulse(TRIG["CP_RESPONSE_PROMPT_ONSET"],
                               global_clock=global_clock)
            ...
            if keys:
                ...
                if picked == 1:
                    trigger_port.pulse_now(TRIG["CP_RESP_1"], global_clock=global_clock)
                    trig_resp = int(TRIG["CP_RESP_1"])
                    t_resp = global_clock.getTime()
                elif picked == 2:
                    trigger_port.pulse_now(TRIG["CP_RESP_2"], global_clock=global_clock)
                    trig_resp = int(TRIG["CP_RESP_2"])
                    t_resp = global_clock.getTime()
                state_current = "state_feedback"
                state_entry = True
            elif resp_clock.getTime() >= RESP_WINDOW_SEC:
                trigger_port.pulse_now(TRIG["CP_RESP_TIMEOUT"],
                              global_clock=global_clock)
