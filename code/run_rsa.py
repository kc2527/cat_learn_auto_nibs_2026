...
if __name__ == "__main__":
    ...
    while running:
        ...
        # --------------------- STATE: BLOCK INTRO ---------------------
            ...
            if keys:
                if current_block != 0:
                    trigger_port.flip_pulse(TRIG["RSA_READY_ONSET"],
                                   global_clock=global_clock)
                    win.flip()
                trigger_port.pulse_now(TRIG["RSA_BLOCK_START"],
                              global_clock=global_clock)
        ...
        # --------------------- STATE: TRIAL PREP ---------------------
            ...
            if state_entry:
                ...
                if trial >= len(schedule):
                    trigger_port.pulse_now(TRIG["RSA_BLOCK_END"],
                                  global_clock=global_clock)
        ...
        # --------------------- STATE: STIM ---------------------
            ...
            if state_entry:
                state_clock.reset()
                trigger_port.flip_pulse(TRIG["RSA_STIM_ONSET"], global_clock=global_clock)
